...
- Docker deployment is still Next.js-only: `deploy/docker` requires the `nextjs-app` capability, so it is not yet offered for Vite or TanStack Start. Vercel works for all three (it requires only `web-app`).
